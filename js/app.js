...
let mPerUnit = null;
let editMode = false; // false = Mode Utilisation
...
function updateRoutePanel(pathIds) {
  ...
  if (!pathIds || pathIds.length < 2) { routeDistanceEl.textContent = 'Distance totale : —'; return; }
...
document.getElementById('computeRoute').onclick = () => {
  ...
  if (!path) { alert('Aucun chemin trouvé.'); drawPath([]); updateRoutePanel(null); }
  else { drawPath(path); updateRoutePanel(path); }
};
...
document.getElementById('defineScaleBtn').onclick = () => {
  ...
  const s = startSelect.value, t = endSelect.value;
  if (s && t) { const path = dijkstra(s, t); if (path) updateRoutePanel(path); }
};
...
function setEditMode(on) {
  editMode = !!on;
  toggleModeBtn.textContent = on ? 'Basculer en mode utilisation' : 'Basculer en mode édition';
  modeBadge.textContent = on ? 'Mode édition' : 'Mode utilisation';
  modeBadge.classList.toggle('edit', on);

  [btnEdgeMode, btnDeleteEdgeMode, btnDeleteLastPoint, btnDeleteLastEdge, btnClearAllEdges, btnExportJson, btnImport, importInput, poiNameInput]
...
function loadGraph(data) {
  ...
  mPerUnit =
    (data.meta && typeof data.meta.scale_m_per_unit === 'number' && isFinite(data.meta.scale_m_per_unit) && data.meta.scale_m_per_unit > 0)
...
// === Initialisation image de fond (IMPORTANT) ===
function initBackgroundDimensions() {
  ...
  img.onload = () => {
    ...
    // ✅ ratio exact du PNG => plus de “marges invisibles” avec contain
    camera.style.setProperty('--plan-ratio', (PLAN_H / PLAN_W * 100) + '%');

    // ✅ repère SVG calé sur le PNG
    svg.setAttribute('viewBox', `0 0 ${PLAN_W} ${PLAN_H}`);
    svg.setAttribute('preserveAspectRatio', 'xMinYMin meet');

    console.log('[OK] Plan chargé:', PLAN_W, PLAN_H);
    applyCamera();
  };
  img.onerror = () => console.error('[ERREUR] Impossible de charger assets/plan.png');
